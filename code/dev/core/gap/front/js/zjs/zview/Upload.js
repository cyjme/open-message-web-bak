...
class Upload extends View {
    ...
    progressFile() {
        ...
        progressFilePreview.addClass('inline file-item on-uploading');
        progressFilePreview.innerHTML = this.tpl`
            <i class="icon icon-loading loading inline font-xlarge"></i>
        `;
        this.ctn.elem('.img-preview').appendChild(progressFilePreview);
    }

    appendImg(key) {
        let progressFilePreview = this.ctn.elem('.on-uploading');
        ...
        `;
        if (progressFilePreview) {
            this.ctn.elem('.img-preview').removeChild(progressFilePreview);
        }
